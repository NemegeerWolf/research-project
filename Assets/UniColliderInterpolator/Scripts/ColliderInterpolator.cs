...
namespace UniColliderInterpolator
{
    public class ColliderInterpolator : MonoBehaviour
    {
        ...
        [SerializeField] public float _divisionUnitLength = 0.5f;
        ...
        public async void Generate()
        {
            ...
            for (var i = 0; i < positionCount; i++)
            {
                ...
                if (Physics.OverlapSphereNonAlloc(position, _divisionUnitLength / 4f, tmpColliders) == 0)
                {
                    continue;
                }
           ...
           // colliderObject.transform.localScale = Vector3.one *0.93f;
            colliderObject.name = colliderObjectName;

            while (hasColliderPosition.Any(p => p))
            {
                //await Task.Delay(0.1f);
                var combineIndex = 0;   
